            ...
            regex_parts.each_with_index do |part, index|
              ...
              when ?:, ?*
                if index != 0 && regex_parts[index - 1][-1] == ?\\
                  ...
                  regex << (@route.matches_with(part[1,
part.size].to_sym) || '.*?').to_s unless validation_regex
                  code << "\#{args.shift || (options && options.delete(:#{part[1,
part.size]})) || return}"
                  dynamic = true
                end
              else
      ...
      def initialize(route, paths)
        ...
        @path_generators = @paths.map do |p|
          generator = PathGenerator.new(route, p.is_a?(String) ? p : route.path_for_generation,
p.is_a?(Regexp) ? p : nil)
        end
      end
      ...
      def matching_path(params, other_hash = nil)
        ...
        when Hash
          @path_generators.find { |path|
            (params && !params.empty? && (path.param_names & params.keys).size == path.param_names.size) || path.param_names.empty?
          }
        end
      end
