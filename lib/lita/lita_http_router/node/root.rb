  ...
  class HttpRouter
    class Node
      class Root < Node
        ...
        def uncompile
          instance_eval "undef :call; def call(req); raise 'uncompiled root'; end", __FILE__,
  __LINE__ if compiled?
        end
        ...
        def add_route(route)
          paths = if route.path_for_generation.nil?
            ...
            until chars.empty?
              ...
              when ?(
                chars.shift
                (start_index...end_index).each { |path_index|
                  raw_paths << raw_paths[path_index].dup
                }
              ...
              when ?)
                ...
                c = if chars[0][0] == ?\\ && (chars[1][0] == ?( || chars[1][0] == ?));
                  chars.shift; chars.shift; else; chars.shift; end
          ...
          if paths.empty?
            ...
            Generator.new(route, paths).each_path do |path_generator|
              ...
              when Regexp
                ...
                add_non_path_to_tree(route, add_free_match(path_generator.path),
  path_generator.path, path_generator.param_names)
              else
                node = self
                path_generator.path.split(/\//).each do |part|
                  ...
                  node = parts.size == 1 ? add_normal_part(route, node, part,
  path_generator) : add_complex_part(route, node, parts, path_generator)
                end
        ...
        def add_complex_part(route, node, parts, path_generator)
          ...
          spans ? node.add_spanning_match(Regexp.new("#{regex}$"), capturing_indicies,
  splitting_indicies) :
        ...
        def add_non_path_to_tree(route, node, path, param_names)
          node = node.add_host([route.host,
                                route.other_hosts].flatten.compact) if route.host or route.other_hosts
