...
require 'cgi'
require 'url_mount'
require_relative 'lita_http_router/node'
require_relative 'lita_http_router/request'
require_relative 'lita_http_router/response'
require_relative 'lita_http_router/route'
require_relative 'lita_http_router/generator'
require_relative 'lita_http_router/route_helper'
require_relative 'lita_http_router/generation_helper'
require_relative 'lita_http_router/regex_route_generation'
require_relative 'lita_http_router/util'
...
module Lita
  class HttpRouter
    ...
    def initialize(*args, &blk)
      ...
      @options                 = options
      @default_app             = default_app || options && options[:default_app] || proc { |env|
        ::Rack::Response.new("Not Found", 404, { 'X-Cascade' => 'pass' }).finish
      }
    ...
    def reset!
      ...
      @default_app = Proc.new { |env|
        ::Rack::Response.new("Your request couldn't be found", 404).finish
      }
    ...
    def rewrite_partial_path_info(env, request)
      ...
      env['SCRIPT_NAME'] += request.rack_request.path_info[0,
request.rack_request.path_info.size - env['PATH_INFO'].size]
    end
    ...
    def raw_url(route, *args)
      case route
      when Symbol then @named_routes.key?(route) && @named_routes[route].each { |r|
                         url = r.url(*args); return url if url
                       }
    ...
    def raw_url_ns(route, *args)
      case route
      when Symbol then @named_routes.key?(route) && @named_routes[route].each { |r|
                         url = r.url_ns(*args); return url if url
                       }
    ...
    def raw_path(route, *args)
      case route
      when Symbol then @named_routes.key?(route) && @named_routes[route].each { |r|
                         path = r.path(*args); return path if path
                       }
